...
\begin{algoprob}
	...
	\probleminput{$\mcU$: Universe of all elements $u_1,\dots, u_n$
		...
		$\mcS=\{S_1,\dots, S_m\}$, $S_i\subseteq \mcU$ for all $i\in[m]$
...
\nt{$O(\log n)$-approximation is also the best we can do for set cover. Doing better than that is $\mathsf{NP}$-hard.}

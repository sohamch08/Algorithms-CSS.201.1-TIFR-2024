...
\begin{Definition}{Maximal Matching}{}
	For a graph $G=(V,E)$ a matching $M\subseteq E$ is maximal if it cannot be extended by adding an edge.
\end{Definition}
...
\solve{ Clearly algorithm output is not optimal always. We get a maximal matching sure. But we don't get a maximum matching always. For example the following graph
 ...
 Since the algorithm output may not always be optimal we can ask the following question
 \begin{question}{}{}
...
\begin{algoprob}
	...
	\probleminput{$n$ symbols $A=(a_1,\dots, a_n)$ and their frequencies $P=(f_1,\dots, f_n)$ of using symbols}
...
\begin{algoprob}
	...
	\problemquestion{Each job takes unit time and we have a single machine to process the jobs. Give a feasible schedule of jobs with maximum reward}
\end{algoprob}
...
\begin{Definition}{Feasible Schedule}{}
	...
	\item A feasible schedule is one where every job in $S$ gets finished by its deadline.

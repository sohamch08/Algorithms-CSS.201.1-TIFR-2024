...
\begin{algoprob}
	...
	\problemquestion{Given a set of points, find the closest pair of points in $\bbR^2$, i.e. find $P_i,P_j$ that are at minimum $l_2$ distance i.e. minimize $\sqrt{(x_i-x_j)^2+(y_i-y_j)^2}$.}
\end{algoprob}

\section{Naive Algorithm}

Now the naive algorithm for this will be checking all pairs of points and take their distance and output the minimum one. There are total $\binom{n }{2}$ possible choices of pairs of points. And calculating the distance of each pair takes $O(1)$ time. So it will take $O(n^2)$ time to find the closest pair of points. \parinf
...
\begin{algorithm}
	...
	\KwOut{Closest pair of points, $(P_i,P_j,\delta)$ where $\delta=d(P_i,P_j)$}
	\Begin{
...
This algorithm only sorts one time. So time complexity for $\textsc{Find-Closest-Sorted}(S^x,S^y)$ is $$T(n)=2T\lt(\frac{n}{2}\rt)+O(n)\implies T(n)=O(n\log n)$$and therefore time complexity for $\textsc{Find-Closest}(S)$ is $O(n\log n)$.

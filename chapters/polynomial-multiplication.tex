...
\end{question}
We can rewrite $A(x)$ as $$A(x)=a_0+x(a_1+x(a_2+x(a_3+\cdots (a_{n-1}+x(a_n))\cdots )))$$In this representation it is clear that we have to do $n$ additions and $n$ multiplications to find $A(x_0)$. Hence, we can evaluate an $n-1$ degree polynomial at a point in $O(n)$ time
...
Let $$\om_n^k=n^{th}\text{ root of unity for }k\in\llbracket n-1\rrbracket = e^{i\frac{k}{n}2\pi}=\cos\lt(\frac{k}{n}2\pi\rt)+i\sin\lt(\frac{k}{n}2\pi\rt)$$Hence we have \begin{align*}
